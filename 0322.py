...
# 定義輸入和輸出檔案路徑
input_file = "313706038_0406_2.json"
output_file = "313706038_4.json"

# 讀取 JSON 檔案
with open(input_file, 'r', encoding='utf-8') as f:
    data = json.load(f)

# 處理每個條目，移除 abstract 中的換行符號
for entry in data:
    if 'abstract' in entry and entry['abstract']:
        # 將換行符號替換為空格
        entry['abstract'] = entry['abstract'].replace('\n', ' ')

# 將修改後的數據寫入新檔案，無縮排、無上下 []，且每行末尾無逗號
...
print(f"已完成處理，結果已儲存至 {output_file}")


# In[1]:


pip freeze | grep -E 'torch|transformers|peft|datasets|evaluate'
